...
namespace ARKOM.Enemy
{
    ...
    public class ChasingGhost : MonoBehaviour
    {
        ...
        [Tooltip("???????????????????????????? (???????????????????????????????)")] public Transform catchCamAnchor;
        [Tooltip("??????????/???????????????????? (???? ????/?????)")] public Transform holdAnchor;
        [Header("Animation Triggers")]
        [Tooltip("???? Trigger ?? Animator ???????????????????????")] public string catchTriggerParam = "Catch";

        [Header("Speed")] public float walkSpeed = 2.8f; public float runSpeed = 4.2f;

        [Header("Animation Params")]
        [Tooltip("???????????????????????? 2D (X/Y) ?????? BlendTree Idle/Walk/Strafe")] public bool use2DLocomotionParams = false;
        [Tooltip("??????????????? Animator ??? X (???????????; Idle ?????? 0)")] public string xParam2D = "X";
        [Tooltip("??????????????? Animator ??? Y (???????????; ?????? Idle/????)")] public string yParam2D = "Y";
        [Tooltip("????????????????????????? Float (???? Speed) ?????? BlendTree Idle/Walk")] public bool useFloatSpeedParam = true;
        [Tooltip("??????????????? Float ????????")] public string speedParam = "Speed";
        [Tooltip("??????????????? Bool (??????????? Float) ???????????????????")] public string walkBoolParam = "IsWalking";
        [Tooltip("??? Threshold ??????????????????? '????' (???????? Bool)")] public float walkStartThreshold = 0.1f;
        [Tooltip("????????????????????????????? Animator")] public float speedAnimScale = 1.0f;

        [Header("Detect (Vision)")]
        [Tooltip("???????????? (?????????????????)")] public float detectionRadius = 12f;
        [Tooltip("??????????? (FOV/2) ?????????????")] public float fovHalfAngle = 55f;
        [Tooltip("LayerMask ?????????????? (Raycast ??????????)")] public LayerMask losBlockMask = ~0;
        [Tooltip("???????????????????????????????????????")] public float confirmTime = 0.2f;

        [Header("Detect (Misc)")]
        [Tooltip("??????????? (?????? ????????????? ??????????? ?)")] public float hearRadius = 0f;

        [Header("Chase/Leash")]
        [Tooltip("?????????????????????? (???????????????????????)")] public float chaseStartDistance = 8f;
        [Tooltip("??????????/????????????????????")] public float leashRadius = 25f;
        [Tooltip("????????????????????????????")] public float lostTargetTimeout = 3.0f;
        [Tooltip("????????????????????")] public float catchDistance = 1.4f;
        [Tooltip("???????????????????????????????")] public float repathInterval = 0.25f;
        [Tooltip("???????????????????????????????????? (?????????????)")] public float wanderOffset = 0.5f;

        [Header("Patrol")]
        public Transform homeAnchor; // ???????/??????????????????
        public Transform[] patrolWaypoints;
        [Tooltip("???????????? (???????????????????????)")] public bool patrolLoop = true;
        [Tooltip("???????????????????????")] public float patrolWaitAtPoint = 0.5f;
        [Tooltip("?????????????????????????")] public float waypointArriveDistance = 0.5f;

        [Header("Search Settings")] public bool onlyInvestigateWhenChasing = true;
        [Tooltip("??????????????????????????????????????")] public float searchHoldTime = 3.0f;

        [Header("Audio")]
        [Tooltip("??????????????????????????")] public AudioClip catchSfx;
        ...
        private void UpdateAnimator()
        {
            ...
            {
                // ????????????? local (????/????) ??????? 2D BlendTree X/Y ?????? Idle/Walk
