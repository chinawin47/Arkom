...
namespace ARKOM.Scenes.Road
{
    public class StreetLightFlicker : MonoBehaviour
    {
        ...
        [Header("Audio")]
        // ถ้าเปิดเล่นเสียงตอนเริ่ม/จบการกระพริบ
        [Tooltip("เปิดเล่นเสียงตอนเริ่ม/จบการกระพริบ")] public bool enableAudio = true;

        // เสียงตอนเริ่ม burst (ไฟเริ่มกระพริบ)
        [Tooltip("เสียงตอนเริ่ม burst (ไฟเริ่มกระพริบ)")] public AudioClip flickerStartClip;
        [Range(0f, 1f)] public float flickerStartVolume = 1f;

        // เสียงตอนจบ burst (ไฟกลับนิ่ง)
        [Tooltip("เสียงตอนจบ burst (ไฟกลับนิ่ง)")] public AudioClip flickerEndClip;
        [Range(0f, 1f)] public float flickerEndVolume = 1f;

        // เสียง pop เล็กๆ เมื่อเกิด hard blink (สุ่มบางครั้ง)
        [Tooltip("เสียง pop เล็กๆ เมื่อเกิด hard blink (สุ่มบางครั้ง)")] public AudioClip hardBlinkClip;
        [Range(0f, 1f)] public float hardBlinkClipProbability = 0.25f;

        // คูลดาวน์ขั้นต่ำระหว่างเสียง hardBlink (วินาที)
        [Tooltip("คูลดาวน์ขั้นต่ำระหว่างเสียง hardBlink (วินาที)")] public float hardBlinkClipCooldown = 0.15f;

        // ตำแหน่งเล่นเสียง3D (เว้นว่างใช้ตำแหน่งเฉลี่ยของไฟ)
        [Tooltip("ตำแหน่งเล่นเสียง3D (เว้นว่างใช้ตำแหน่งเฉลี่ยของไฟ)")] public Transform audioPlayPoint;

        // ใช้ระบบ AudioManager (ถ้า false จะสร้าง AudioSource ภายใน)
        [Tooltip("ใช้ระบบ AudioManager (ถ้า false จะสร้าง AudioSource ภายใน)")] public bool useGlobalAudioManager = true;

        // สร้าง AudioSource ภายในสำหรับเสียง (ถ้าจำเป็น)
        [Tooltip("สร้าง AudioSource ภายในสำหรับเสียง (ถ้าจำเป็น)")] public bool createLocalAudioSource = true;

        // ปรับ pitch สุ่มเล็กน้อยให้ start/end (min,max)
        [Tooltip("ปรับ pitch สุ่มเล็กน้อยให้ start/end (min,max)")] public Vector2 pitchJitter = new Vector2(0.95f, 1.05f);

        private AudioSource localSource;
        private float lastHardBlinkSoundTime;
        ...
        private void Awake()
        {
            ...
            CacheLights();
            if (createLocalAudioSource && !useGlobalAudioManager)
            {
        ...
        private void OnValidate()
        {
            ...
            if (pitchJitter.x > pitchJitter.y) pitchJitter.y = pitchJitter.x;
        }
        ...
        {
            // play start sound
            PlayClip(flickerStartClip, flickerStartVolume);
            ...
            while (t < duration)
            {
                ...
                for (int i = 0; i < lights.Length; i++)
                {
                    ...
                    bool didHardBlink = false;
                    if (Random.value < hardBlinkChance * Time.deltaTime * speed)
                    {
                        mul = 0f; didHardBlink = true;
                    }
                    lights[i].intensity = baseIntensity[i] * mul;
                    if (didHardBlink && hardBlinkClip && enableAudio && Time.time - lastHardBlinkSoundTime >= hardBlinkClipCooldown)
                    {
                        if (Random.value <= hardBlinkClipProbability)
                        {
                            lastHardBlinkSoundTime = Time.time;
                            PlayClip(hardBlinkClip, flickerStartVolume * 0.8f);
                        }
                    }
                }
                yield return null;
            }
            // restore and play end sound
            for (int i = 0; i < lights.Length; i++) if (lights[i]) lights[i].intensity = baseIntensity[i];
            PlayClip(flickerEndClip, flickerEndVolume);
            routine = null;
        }
        ...
        {
            if (audioPlayPoint) return audioPlayPoint.position;
            if (lights != null && lights.Length > 0)
            {
                Vector3 sum = Vector3.zero; int count = 0;
                for (int i = 0; i < lights.Length; i++) { if (!lights[i]) continue; sum += lights[i].transform.position; count++; }
        ...
        {
            if (!enableAudio || !clip) return;
            float pitch = Random.Range(pitchJitter.x, pitchJitter.y);
            if (useGlobalAudioManager && ARKOM.Audio.AudioManager.Instance)
            {
                var src = ARKOM.Audio.AudioManager.Instance.Play3D(clip, GetAudioPosition(), vol, pitch, pitch, 2f, 30f);
            }
            else if (localSource)
            {
                localSource.pitch = pitch; localSource.PlayOneShot(clip, vol);
            }
        }
    }
}
